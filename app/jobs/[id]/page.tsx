...
import {
  FaArrowLeft, FaBuilding, FaLocationDot, FaBriefcase, FaClock,
  FaUsers, FaArrowUpRightFromSquare, FaXmark, FaCheck, FaUser,
  FaFileLines, FaGlobe, FaTriangleExclamation,
...
function AdGate({ onComplete }: { onComplete: (watched: boolean) => void }) {
  const [adStatus, setAdStatus] = useState<'detecting' | 'filled' | 'unfilled'>('detecting')
  const [seconds, setSeconds] = useState(5)
  const [ready, setReady] = useState(false)
  ...
  useEffect(() => {
    ...
    const checkStatus = () => {
      ...
      if (status === 'unfilled') resolve(false)
    }
    ...
    const statusPoll = setInterval(checkStatus, 250)
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="bg-white w-full sm:max-w-lg rounded-t-3xl sm:rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-scaleIn">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-grey4/60 shrink-0">
          <p className="font-satoshi font-semibold text-sm text-grey1">{stepTitle}</p>
        ...
        {step === 'no_resumes' && (
          <div className="p-8 text-center flex-1">
            ...
            <p className="font-satoshi font-bold text-xl text-grey1 mb-2">Add a resume first</p>
            <p className="font-openSans text-sm text-grey3 mb-8">
              You need at least one resume on your profile before you can apply. Upload one in your dashboard settings — it only takes a minute.
            </p>
            <div className="flex flex-col gap-2">
              ...
              <button onClick={onClose} className="w-full py-3 border border-grey4 text-grey1 rounded-lg font-satoshi font-semibold text-sm hover:bg-grey5 transition-colors">
                Cancel
              </button>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto">
              ...
              <div className="px-6 py-5 border-b border-grey4/60 bg-grey6/50">
                ...
                <div className="flex items-center gap-3">
                  <div className="w-11 h-11 rounded-full overflow-hidden bg-grey5 flex items-center justify-center shrink-0">
                    {profile.profile_image_url ? (
                      <Image src={profile.profile_image_url} alt={profile.name ?? ''} width={44} height={44} className="object-cover w-full h-full" />
                    ) : (
                ...
                <div className="flex items-center gap-3 mt-3">
                  {profile.portfolio_or_website_link && (
                    <a href={profile.portfolio_or_website_link} target="_blank" rel="noopener noreferrer"
            ...
            <div className="px-6 py-5 border-b border-grey4/60">
              ...
              {resumes.length === 1 ? (
                <div className="flex items-center gap-3 p-3 rounded-lg border-2 border-mainPurple bg-lightPurple">
                  ...
                  <span className="font-satoshi font-semibold text-sm text-mainPurple">{resumes[0].label}</span>
                </div>
              ) : (
                  ...
                  {resumes.map((r) => (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 p-3 rounded-lg border-2 text-left transition-all ${
                        selectedResumeId === r.id ? 'border-mainPurple bg-lightPurple' : 'border-grey4/60 hover:border-grey3'
                      }`}
                    >
                      <FaFileLines className={`w-4 h-4 shrink-0 ${selectedResumeId === r.id ? 'text-mainPurple' : 'text-grey3'}`} />
                      <span className={`font-satoshi font-semibold text-sm ${selectedResumeId === r.id ? 'text-mainPurple' : 'text-grey1'}`}>
                        {r.label}
                      </span>
                      {r.is_default && (
                        <span className="ml-auto text-[10px] font-semibold text-grey3 font-satoshi">Default</span>
                      )}
                    </button>
                  ))}
                </div>
            ...
            <div className="px-6 py-5">
              <label className="block font-satoshi font-semibold text-xs text-grey3 uppercase tracking-wide mb-2">
                Cover Note <span className="normal-case text-grey3 font-normal">(optional)</span>
              </label>
              <textarea
                value={coverNote}
              ...
              />
              <p className="mt-1 text-xs text-grey3 font-openSans tabular-nums text-right">{coverNote.length}/300</p>
            </div>
          </div>
        )}
        ...
        {step === 'success' && (
          <div className="p-8 text-center flex-1">
            ...
            <p className="font-satoshi font-bold text-2xl text-grey1 mb-2">Application sent!</p>
            <p className="font-openSans text-sm text-grey3 mb-8">
              Your Segwae profile has been shared with {job.companies?.name ?? 'the company'}. Track your progress in Applications.
            </p>
            <div className="flex flex-col gap-2">
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      try {
        ...
        if (user) {
          const checkRes = await fetch(`/api/hiring/applications?check_job_id=${id}`)
  ...
  if (!job) {
    return (
      <div className="min-h-screen bg-grey6 flex flex-col items-center justify-center gap-4">
        <p className="font-satoshi font-bold text-2xl text-grey1">Job not found</p>
        <Link href="/jobs" className="text-sm font-satoshi font-semibold text-mainPurple hover:opacity-70 transition-opacity">
          ← Browse all jobs
        </Link>
      </div>
    )
  ...
  const renderCTA = () => {
    if (job.posting_mode === 'external') {
      return (
        ...
        >
          Apply on company website <FaArrowUpRightFromSquare className="w-3 h-3" />
        </a>
      )
    }
    ...
    <div className="min-h-screen bg-grey6">
      <div className="bg-[#111827] px-5 py-4">
        <div className="max-w-4xl mx-auto">
          <Link href="/jobs" className="inline-flex items-center gap-1.5 text-xs font-semibold text-white/50 hover:text-white font-satoshi transition-colors">
      ...
      <div className="max-w-4xl mx-auto px-5 py-8">
        <div className="lg:grid lg:grid-cols-[1fr_320px] lg:gap-8">
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-grey4/60 p-6">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <h1 className="font-satoshi font-bold text-2xl text-grey1">{job.title}</h1>
                  <p className="font-openSans text-sm text-grey2 mt-0.5">{company?.name}</p>
                </div>
              </div>

              <div className="flex items-center gap-2 mt-5 flex-wrap">
                <span className={`text-[11px] font-semibold font-satoshi px-2.5 py-1 rounded-full ${WORK_MODE_COLORS[job.work_mode]}`}>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-5 pt-5 border-t border-grey4/40">
                {job.location && (
                  <div>
                    ...
                    <p className="font-openSans text-sm text-grey1">{job.location}</p>
                  </div>
                )}
                <div>
                  <p className="font-satoshi text-[11px] font-semibold text-grey3 mb-1 flex items-center gap-1">
                  ...
                  <p className="font-openSans text-sm text-grey1">
                    {job.experience_years_min > 0 ? `${job.experience_years_min}+ years` : 'Open to all'}
                  </p>
                </div>
                {salary && (
                  <div>
                    <p className="font-satoshi text-[11px] font-semibold text-grey3 mb-1">Salary</p>
                ...
                {job.posting_mode === 'internal' && (
                  <div>
                    ...
                    <p className="font-openSans text-sm text-grey1">{job.application_count ?? 0}</p>
                  </div>
                )}
              </div>
              ...
              {job.application_deadline && (
                <div className="mt-4 flex items-center gap-2 text-xs font-openSans text-grey3">
                  ...
                  Deadline: {new Date(job.application_deadline).toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}
                </div>
              )}
            </div>
            ...
            {/* Description */}
            <div className="bg-white rounded-2xl border border-grey4/60 p-6">
              <h2 className="font-satoshi font-semibold text-grey1 mb-4">Job Description</h2>
              <div className="font-openSans text-sm text-grey1 leading-relaxed whitespace-pre-wrap">{job.description}</div>
            </div>

            {/* Requirements */}
            {job.requirements && (
              <div className="bg-white rounded-2xl border border-grey4/60 p-6">
                <h2 className="font-satoshi font-semibold text-grey1 mb-4">Requirements</h2>
                <div className="font-openSans text-sm text-grey1 leading-relaxed whitespace-pre-wrap">{job.requirements}</div>
              </div>
            )}
            ...
            {job.tags && job.tags.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {job.tags.map((tag) => (
                  <span key={tag} className="text-xs font-openSans text-grey2 bg-white border border-grey4/60 px-3 py-1.5 rounded-lg">{tag}</span>
                ))}
              </div>
            )}
            ...
            {company && (
              <div className="bg-white rounded-2xl border border-grey4/60 p-6">
                <h2 className="font-satoshi font-semibold text-grey1 mb-4">About {company.name}</h2>
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <p className="font-satoshi font-semibold text-sm text-grey1">{company.name}</p>
                    <div className="flex items-center gap-3 mt-1 flex-wrap">
                      {company.industry && <span className="font-openSans text-xs text-grey3">{company.industry}</span>}
                      {company.size && <span className="font-openSans text-xs text-grey3">{company.size} employees</span>}
                      {company.location && <span className="font-openSans text-xs text-grey3">{company.location}</span>}
                    </div>
                    {company.description && (
                      <p className="font-openSans text-sm text-grey2 mt-3 leading-relaxed">{company.description}</p>
                    )}
                    {company.website && (
          ...
          <div className="hidden lg:block">
            <div className="sticky top-8 space-y-4">
              <div className="bg-white rounded-2xl border border-grey4/60 p-5">
                <p className="font-satoshi font-semibold text-sm text-grey1 mb-4">{job.title}</p>
                {renderCTA()}
              ...
              <div className="bg-white rounded-2xl border border-grey4/60 p-5 space-y-3">
                <div className="flex justify-between text-xs">
                  <span className="font-satoshi font-semibold text-grey3">Posted</span>
                ...
                {job.posting_mode === 'internal' && (
                  <div className="flex justify-between text-xs">
                    <span className="font-satoshi font-semibold text-grey3">Applicants</span>
                    <span className="font-openSans text-grey1">{job.application_count ?? 0}</span>
                  </div>
                )}
                {job.experience_years_min > 0 && (
                  <div className="flex justify-between text-xs">
                    <span className="font-satoshi font-semibold text-grey3">Experience</span>
                    <span className="font-openSans text-grey1">{job.experience_years_min}+ years</span>
                  </div>
                )}
              </div>
            </div>
