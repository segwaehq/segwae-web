...
import { FaPlus, FaUsers, FaArrowRight, FaEllipsisVertical, FaLocationDot } from 'react-icons/fa6'
...
const MODE_LABELS: Record<string, string> = { internal: 'Internal', external: 'External' }
...
function JobRow({ job, onStatusChange }: { job: Job; onStatusChange: () => void }) {
  ...
  const actions = []
  if (job.status === 'draft') actions.push({ label: 'Publish', next: 'active' })
  if (job.status === 'active') actions.push({ label: 'Pause', next: 'paused' })
  if (job.status === 'paused') actions.push({ label: 'Resume', next: 'active' })
  if (job.status !== 'archived') actions.push({ label: 'Archive', next: 'archived' })

  return (
    <tr className="border-b border-grey4/60 hover:bg-grey6/50 transition-colors">
      <td className="py-4 px-5">
        <div className="flex flex-col gap-1">
          <span className="font-satoshi font-semibold text-sm text-grey1">{job.title}</span>
          <div className="flex items-center gap-2 flex-wrap">
            <span className={`text-[10px] font-semibold font-satoshi px-2 py-0.5 rounded-full ${MODE_LABELS[job.posting_mode] === 'External' ? 'text-blue bg-blue/10' : 'text-mainPurple bg-lightPurple'}`}>
              {MODE_LABELS[job.posting_mode]}
            </span>
            <span className="text-[10px] text-grey3 font-openSans">{JOB_TYPE_LABELS[job.job_type]}</span>
            {job.location && (
      ...
      <td className="py-4 px-5">
        <span className={`text-xs font-semibold font-satoshi px-2.5 py-1 rounded-full ${STATUS_COLORS[job.status]}`}>
      ...
      <td className="py-4 px-5 text-xs font-openSans text-grey3 hidden lg:table-cell">{relTime}</td>
      <td className="py-4 px-5">
          ...
          <div className="relative">
            ...
            {menuOpen && (
              <>
                ...
                <div className="absolute right-0 top-8 z-20 bg-white border border-grey4/60 rounded-xl shadow-lg py-1 min-w-[130px]">
                  {actions.map((a) => (
                    <button
                      key={a.next}
    ...
    <div className="max-w-full">
      <div className="mb-8">
        ...
        <div className="flex items-start justify-between gap-4">
          <h1 className="font-satoshi font-bold text-2xl text-grey1">Your Jobs</h1>
          <Link
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        {[
          ...
          { label: 'Total applicants', value: totalApplicants, color: 'text-mainPurple' },
          { label: 'Unresolved', value: unresolved, color: 'text-warningYellow' },
        ].map((s) => (
          <div key={s.label} className="bg-white rounded-2xl border border-grey4/60 p-5">
            <p className={`font-satoshi font-bold text-3xl ${s.color}`}>{s.value}</p>
      ...
      <div className="bg-white rounded-2xl border border-grey4/60 overflow-hidden">
        {jobs.length === 0 ? (
          <div className="py-20 flex flex-col items-center gap-4">
            ...
            <p className="font-openSans text-sm text-grey3">Post your first job listing to start receiving applications</p>
            <Link
              href="/dashboard/hiring/post"
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-grey4/60 bg-grey6/50">
                <th className="text-left py-3 px-5 text-xs font-semibold text-grey3 font-satoshi uppercase tracking-wide">Role</th>
                <th className="text-left py-3 px-5 text-xs font-semibold text-grey3 font-satoshi uppercase tracking-wide">Status</th>
                <th className="text-left py-3 px-5 text-xs font-semibold text-grey3 font-satoshi uppercase tracking-wide hidden md:table-cell">Applicants</th>
                <th className="text-left py-3 px-5 text-xs font-semibold text-grey3 font-satoshi uppercase tracking-wide hidden lg:table-cell">Posted</th>
