...
export default async function BugReportsPage() {
  ...
  const getStatusBadge = (status: string) => {
    const styles = {
      ...
      closed: 'bg-grey3/10 text-grey3 dark:text-content-subtle',
    }
    ...
    <div>
      ...
      {!bugReports || bugReports.length === 0 ? (
        <div className="bg-white dark:bg-surface-raised rounded-2xl p-12 text-center shadow-lg">
          <p className="font-spaceGrotesk text-xl text-grey2 dark:text-content-muted">No bug reports yet</p>
        </div>
      ) : (
        <div className="space-y-4">
          {bugReports.map((bug: BugReport) => (
            <div key={bug.id} className="bg-white dark:bg-surface-raised rounded-2xl p-6 shadow-lg">
              ...
              <p className="font-openSans text-grey1 dark:text-content mb-4 whitespace-pre-wrap">{bug.description}</p>
            </div>
          ))}
        </div>
      )}
